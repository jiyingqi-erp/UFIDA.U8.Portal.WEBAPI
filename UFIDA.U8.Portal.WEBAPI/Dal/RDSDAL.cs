        ...
        public static string Dispatch(Dispatch d)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    decimal num = BasicDAL.ToDec(dataTable.Rows[0]["iFHQuantity"].ToString());
                    decimal num2 = BasicDAL.ToDec(dataTable.Rows[0]["iQuantity"].ToString());
                    if (num + items[i].iQuantity > num2)
                    {
                ...
                for (int j = 0; j < items.Count; j++)
                {
                    ...
                    decimal num5 = BasicDAL.ToDec(U8SqlDBHelper.GetString("select iExchRate from so_somain where csocode='" + items[j].cSoCode + "'"));
                    decimal num6 = BasicDAL.ToDec(dataTable3.Rows[0]["iUnitPrice"].ToString());
                    decimal num7 = BasicDAL.ToDec(dataTable3.Rows[0]["iTaxUnitPrice"].ToString());
                    decimal num8 = BasicDAL.ToDec(dataTable3.Rows[0]["iTaxRate"].ToString());
                    decimal num9 = Math.Round(num7 * iQuantity, 2);
                    decimal num10 = Math.Round(num9 / (100m + num8) * 100m, 2);
                    decimal num11 = num9 - num10;
                    decimal num12 = Math.Round(num10 * num5, 2);
                    decimal num13 = Math.Round(num11 * num5, 2);
                    decimal num14 = Math.Round(num9 * num5, 2);
                    int vouchId2 = BasicDAL.GetVouchId("C", cAcc_Id, "DISPATCH");
                    string iaCreateBillFlag = "1" + $"{vouchId2:D9}";
                    sql = " insert into dispatchlists  (DLID, irowno, iDLsID,bsaleprice,  cInvCode, iQuantity, iNum, iUnitPrice, iTaxUnitPrice,  iMoney, iTax, iSum, iNatUnitPrice, iNatMoney, iNatTax, iNatSum, iSettleNum,  cBatch, bSettleAll, iTB, cInvName, iTaxRate, fOutQuantity, fOutNum, bIsSTQc,  cUnitID, bGsp, bQANeedCheck, bQAUrgency, bQAChecking, bQAChecked, KL, KL2,   cordercode,iorderrowno,iSOsID,cSoCode, cItemCode,cItemName,cItem_class,cItem_CName,  dMDate,dvDate,cExpirationdate, iMassDate, cMassUnit, bcosting, bIAcreatebill, bneedsign, cWhCode ,  cDefine22,cDefine23,cDefine24,cDefine25,cDefine26,cDefine27,cDefine28,cDefine29,  cDefine30,cDefine31,cDefine32,cDefine33,cDefine34,cDefine35,cDefine36,cDefine37 ) \r\n select   '" + detailId + "'," + num4 + ",'" + iaCreateBillFlag + "',1,  '" + cInvCode + "', " + iQuantity + "," + iNum + ", " + num6 + "," + num7 + ",  " + num10 + ", " + num11 + ", " + num9 + ",iNatUnitPrice , " + num12 + ", " + num13 + "," + num14 + ", 0 ,  " + batchValue + ",0,0, cInvName , iTaxRate , 0 ," + iNum + ",0,  cUnitID ,0,0,0,0,0, KL, KL2,  cSoCode , iRowNo, iSOsID, cSoCode, cItemCode,cItemName,cItem_class,cItem_CName,  " + posCodeValue + "," + madeDateValue + "," + vDateValue + "," + expDateValue + "," + massDateValue + ", 1," + batchClause + ",0, " + value3 + ",  '" + items[j].cDefine22 + "','" + items[j].cDefine23 + "',cDefine24,cDefine25,cDefine26,cDefine27,cDefine28,cDefine29,  cDefine30,cDefine31,cDefine32,cDefine33,cDefine34,cDefine35,cDefine36,cDefine37 \r\n from so_sodetails  where cSoCode = '" + items[j].cSoCode + "' and iRowNo = '" + items[j].Irowno + "' ";
                    list.Add(sql);
                    fEntryList.Add("{\"FEntryID\":\"" + iaCreateBillFlag + "\",\"Fseq\":" + num4 + "}");
                    sql = " update so_sodetails set iFHQuantity=isnull(iFHQuantity,0)+ " + iQuantity + ",   iFHNum=isnull(iFHNum,0)+isnull(" + iNum + ",0) , iFHMoney=isnull(iFHMoney,0)+ " + num10 + "  where cSoCode = '" + items[j].cSoCode + "' and iRowNo = '" + items[j].Irowno + "' ";
                    list.Add(sql);
                    string batchClause2 = U8SqlDBHelper.GetString(" select cBusType from SO_SOMain where cSOCode = '" + items[j].cSoCode + "'");
                    sql = " insert into IA_SA_UnAccountVouch (IDUN,IDSUN,cVouTypeUN,cBustypeUN)  values ('" + detailId + "','" + iaCreateBillFlag + "','05','" + batchClause2 + "' ) ";
                    list.Add(sql);
                }
                int num15 = U8SqlDBHelper.ExecuteSqlTran(list);
                if (num15 > 0)
                {
