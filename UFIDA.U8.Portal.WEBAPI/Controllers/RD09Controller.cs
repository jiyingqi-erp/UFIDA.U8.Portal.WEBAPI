...
{
    public class RD09Controller : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				rd = JsonConvert.DeserializeObject<RD09>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("RD09", requestData, responseData);
		}
	}
}
}
