...
{
    public class OrderUptSubsController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    um = JsonConvert.DeserializeObject<UptMom>(requestData);
                }
                catch
                {
                ...
                responseData = MomOrderDAL.UptSubs(um);
                return new HttpResponseMessage
                {
                    Content = new StringContent(responseData, Encoding.UTF8, "application/json")
                };
            ...
            {
                LogException.WriteJSlog("OrderUptSubs", requestData, responseData);
            }
        }
    }
}
