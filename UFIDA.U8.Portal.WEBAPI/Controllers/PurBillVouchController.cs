...
{
    public class PurBillVouchController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				p = JsonConvert.DeserializeObject<PurBill>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("PurBillVouch", requestData, responseData);
		}
	}
}
}
