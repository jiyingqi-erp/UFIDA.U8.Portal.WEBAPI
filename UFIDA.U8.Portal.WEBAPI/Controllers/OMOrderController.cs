...
{
    public class OMOrderController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				om = JsonConvert.DeserializeObject<OMOrder>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("OMOrder", requestData, responseData);
		}
	}
}
}
