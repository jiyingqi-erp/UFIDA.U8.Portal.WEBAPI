...
{
    public class DispatchUpdateController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    d = JsonConvert.DeserializeObject<Dispatch>(requestData);
                }
                catch
                {
                ...
                responseData = RDSDAL.DispatchUpdate(d);
                return new HttpResponseMessage
                {
                    Content = new StringContent(responseData, Encoding.UTF8, "application/json")
                };
            ...
            {
                LogException.WriteJSlog("DispatchUpdate", requestData, responseData);
            }
        }
    }
}
