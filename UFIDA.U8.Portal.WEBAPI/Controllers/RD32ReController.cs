...
{
    public class RD32ReController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				rd = JsonConvert.DeserializeObject<RD32>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("RD32Re", requestData, responseData);
		}
	}
}
}
