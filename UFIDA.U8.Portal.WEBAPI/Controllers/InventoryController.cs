...
namespace UFIDA.U8.Portal.WEBAPI.Controllers
{
	...
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			VouchsItem vouchsItem;
			try
			{
				vouchsItem = JsonConvert.DeserializeObject<VouchsItem>(requestData);
				DateTime dateTime = Convert.ToDateTime(vouchsItem.VouchTimeS);
				DateTime dateTime2 = Convert.ToDateTime(vouchsItem.VouchTimeE);
			}
			catch
			{
			...
			responseData = DataDownLoadDAL.GetInventory(vouchsItem);
			return new HttpResponseMessage
			{
				Content = new StringContent(responseData, Encoding.UTF8, "application/json")
			};
		...
		{
			LogException.WriteJSlog("Inventory", requestData, responseData);
		}
	}
}
}
