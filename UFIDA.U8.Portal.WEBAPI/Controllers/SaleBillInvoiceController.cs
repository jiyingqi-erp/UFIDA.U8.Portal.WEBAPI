...
{
    public class SaleBillInvoiceController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    sbi = JsonConvert.DeserializeObject<SaleBillInvoice>(requestData);
                }
                catch
                {
                ...
                responseData = InvoiceDal.SaleBillInvoiceBackfill(sbi);
                return new HttpResponseMessage
                {
                    Content = new StringContent(responseData, Encoding.UTF8, "application/json")
                };
            ...
            {
                LogException.WriteJSlog("SaleBillInvoice", requestData, responseData);
            }
        }
    }
}
