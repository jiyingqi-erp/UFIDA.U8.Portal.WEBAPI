...
{
    public class UPPo_PomainController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				p = JsonConvert.DeserializeObject<Pomain>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("UPPo_Pomain", requestData, responseData);
		}
	}
}
}
