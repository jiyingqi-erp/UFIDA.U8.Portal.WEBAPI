...
{
    public class OMRD11Controller : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				rd = JsonConvert.DeserializeObject<RD11>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("OMRD11", requestData, responseData);
		}
	}
}
}
