...
{
    public class SO_UptController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    so = JsonConvert.DeserializeObject<SoMain>(requestData);
                }
                catch
                {
            ...
            {
                LogException.WriteJSlog("SO_Upt", requestData, responseData);
            }
        }
    }
}
