...
{
    public class RD08Controller : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				rd = JsonConvert.DeserializeObject<RD08>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("RD08", requestData, responseData);
		}
	}
}
}
