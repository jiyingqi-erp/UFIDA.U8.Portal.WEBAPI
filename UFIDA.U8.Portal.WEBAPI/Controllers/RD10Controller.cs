...
{
    public class RD10Controller : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				rd = JsonConvert.DeserializeObject<RD10>(requestData);
			}
			catch
			{
		...
		{
			LogException.WriteJSlog("RD10", requestData, responseData);
		}
	}
}
}
