...
{
    public class SaleOrderChangeVerifyController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    so = JsonConvert.DeserializeObject<SaleOrder>(requestData);
                }
                catch
                {
                ...
                responseData = SaleOrderDal.SaleOrderChangeVerify(so);
                return new HttpResponseMessage
                {
                    Content = new StringContent(responseData, Encoding.UTF8, "application/json")
                };
            ...
            {
                LogException.WriteJSlog("SaleOrderChangeVerify", requestData, responseData);
            }
        }
    }
}