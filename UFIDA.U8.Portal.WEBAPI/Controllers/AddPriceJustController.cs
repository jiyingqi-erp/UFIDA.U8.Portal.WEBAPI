...
{
    public class AddPriceJustController : ApiController
    {
        public HttpResponseMessage Post([FromBody] dynamic json)
        {
            ...
            try
            {
                ...
                try
                {
                    p = JsonConvert.DeserializeObject<PuPriceJust>(requestData);
                }
                catch
                {
                ...
                responseData = PoDAL.AddPriceJust(p);
                return new HttpResponseMessage
                {
                    Content = new StringContent(responseData, Encoding.UTF8, "application/json")
                };
            ...
            {
                LogException.WriteJSlog("AddPriceJust", requestData, responseData);
            }
        }
    }
}