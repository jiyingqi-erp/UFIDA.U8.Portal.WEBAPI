...
{
    public class VenInvController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				vai = JsonConvert.DeserializeObject<VenAndInv>(requestData);
			}
			catch
			{
			...
			responseData = PoDAL.VenAndInv(vai);
			return new HttpResponseMessage
			{
				Content = new StringContent(responseData, Encoding.UTF8, "application/json")
			};
		...
		{
			LogException.WriteJSlog("VenInv", requestData, responseData);
		}
	}
}
}
