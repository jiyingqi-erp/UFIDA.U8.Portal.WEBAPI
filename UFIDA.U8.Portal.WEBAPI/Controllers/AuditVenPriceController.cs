...
{
    public class AuditVenPriceController : ApiController
    {
	public HttpResponseMessage Post([FromBody] dynamic json)
	{
		...
		try
		{
			...
			try
			{
				p = JsonConvert.DeserializeObject<PriceJust>(requestData);
			}
			catch
			{
			...
			responseData = PoDAL.VerifyPrice(p);
			return new HttpResponseMessage
			{
				Content = new StringContent(responseData, Encoding.UTF8, "application/json")
			};
		...
		{
			LogException.WriteJSlog("AuditVenPrice", requestData, responseData);
		}
	}
}
}
